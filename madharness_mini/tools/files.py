...
def list_files(ctx: ToolContext, args: dict[str, Any]) -> dict[str, Any]:
    ...
    for path in paths:
        if path.is_file() and not ignored(path) and fnmatch.fnmatch(path.name, pattern):
...
# Объявляем спецификацию инструментов для работы с файлами
