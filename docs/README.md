# Финальная документация

Эта папка описывает полную версию `madharness-mini` на ветке `main`. Для
учебных срезов открывайте `docs/README.md` в нужной ветке: там индекс ведёт
только к документам соответствующего этапа.

- [Возможности полной версии](capabilities.md)
- [Структура кода](code-overview.md)
- [Слой контекста](context-layer.md)
- [Как пользоваться context_heatmap](context-heatmap.md)
- [Agent Skills](agent-skills.md)
- [MCP](mcp.md)
- [Субагенты](subagents.md)
- [Hooks](hooks.md)
- [Инструмент apply_patch](apply-patch.md)
