...
только стандартная библиотека Python. Клиент говорит только на актуальной
...
9. Каждый MCP tool превращается в локальный `ToolSpec`.
10. `ToolRegistry` добавляет эти specs рядом со встроенными tools.
11. Модель видит MCP tools в обычном поле `tools` OpenAI-совместимого запроса.
...
`requested_inputs`. Если ломается сам transport или JSON-RPC,
`ToolRegistry.call()` ловит исключение и возвращает обычный `fail()` observation.
