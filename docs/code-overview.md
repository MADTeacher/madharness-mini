...
| `madharness_mini/trace.py` | Записывает JSONL-журнал запуска и строит короткую сводку по трассе. |
...
4. `base_messages()` берёт системный промпт из `prompts/system.md` и добавляет инструкции `AGENTS.md`, найденные через `load_agents_md()`.
...
8. Инструменты используют `Policy`, чтобы проверить путь или shell-команду перед доступом к локальному проекту.
...
Основные сценарии покрыты в `tests/test_core.py`: загрузка конфигурации, переопределение через `.env`, политика путей и shell-команд, формат наблюдений инструментов, чтение файлов и порядок загрузки `AGENTS.md`.
