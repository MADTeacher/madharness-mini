...
4. `base_messages()` берёт системный промпт из `prompts/system.md` и добавляет найденные проектные инструкции `AGENTS.md`.
...
7. Если модель вернула `tool_calls`, `ToolRegistry.call()` находит `ToolSpec` по имени и вызывает handler инструмента.
8. Handler получает `ToolContext` с `Config` и `Policy`, чтобы проверить путь относительно проекта или shell-команду перед ее запуском.
...
Новые встроенные инструменты добавляются в пакет `madharness_mini/tools/`: handler и `ToolSpec` лежат рядом в смысловом модуле, а общий список подключается в `builtins.py`. Автообнаружения и внешних плагинов нет, чтобы поток выполнения оставался явным.
